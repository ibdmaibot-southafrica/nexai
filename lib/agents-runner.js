...
export async function runAllAgents({ skip = ["coding"] } = {}) {
  ...
  // CEO always runs first (it sets strategy the others follow). The rest fill the
  // remaining slots via round-robin so every agent gets its turn over time.
  const others = keys.filter((k) => k !== "ceo");
  let cursor = parseInt(await getSetting("agent_cursor", "0")) || 0;
  if (others.length === 0) cursor = 0;
  else if (cursor >= others.length) cursor = 0;
  const rotated = others.slice(cursor).concat(others.slice(0, cursor));
  const rest = rotated.slice(0, Math.max(0, PER_CYCLE - 1));
  const window = keys.includes("ceo") ? ["ceo", ...rest] : rotated.slice(0, PER_CYCLE);
  await setSetting("agent_cursor", String(others.length ? (cursor + rest.length) % others.length : 0));
