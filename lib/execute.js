...
export async function runProduct({ productId, input, key, free = false }) {
  ...
  // Digital IP product: the deliverable is produced once and handed over as-is.
  // No LLM execution — the buyer pays once and receives the finished asset.
  const isDigital = product.deliveryType === "digital" || (!!product.deliverable && !product.systemPrompt);
  if (isDigital) {
    if (!product.deliverable) return { status: 400, error: "Product has no deliverable" };
    // Free preview: first ~600 chars only, so the full IP stays behind payment.
    if (free) {
      const preview = product.deliverable.slice(0, 600);
      return { status: 200, product: product.name, preview, truncated: product.deliverable.length > 600, free: true, delivery: "digital" };
    }
    if (!key) {
      return { status: 401, needsKey: true, error: "API key required", price: product.price, getKey: "POST /api/keys/create { amount } then pass your key" };
    }
    ...
    if (!charge.ok) return { status: 402, error: "Insufficient credits", balance: charge.balance, price: product.price, topUp: "/api/keys/create" };

    const order = await createOrder({
      productId: product.id, buyer: key.slice(0, 12) + "…", amount: product.price,
      currency: product.currency, channel: "credits", status: "paid", deliverable: product.deliverable,
    });
    await logAction("Store", "ip_sale", { product: product.id, price: product.price, orderId: order.id });
    return { status: 200, product: product.name, deliverable: product.deliverable, delivery: "digital", orderId: order.id, charged: product.price, creditsRemaining: charge.balance };
  }

  if (!product.systemPrompt) return { status: 400, error: "Product is not deliverable" };
