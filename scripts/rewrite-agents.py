...
# FINANCE AGENT - Financial review + forecasting (does NOT create invoices).
# Invoices/orders are only ever born from a real checkout (POST /api/payments/create)
# and auto-confirmed "paid" by the PayPal webhook. No fabricated invoices here.
...
export async function runFinanceCycle() {
  ...
  try {
    ...
    // WORK: Financial review and forecasting (no invoice creation)
