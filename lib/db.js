...
export async function getStatus() {
  await init();
  const pool = getPool();
  const [agentsRes, pipelineRes, invoicesRes, financialsRes, strategyRes, logsRes, productsRes] = await Promise.all([
    ...
    // Live storefront + real sales, so the CEO reasons over what's actually
    // selling — not just the pipeline plan. LEFT JOIN orders for real revenue.
    pool.query(`SELECT p.*, COUNT(o.id)::int AS sales, COALESCE(SUM(o.amount),0)::float AS sales_revenue
                FROM products p LEFT JOIN orders o ON o.product_id = p.id
                GROUP BY p.id ORDER BY p.created_at DESC`).catch(() => ({ rows: [] })),
  ]);
  ...
  return {
    ...
    products: productsRes.rows.map(p => ({ id: p.id, name: p.name, status: p.status, price: Number(p.price), category: p.category, deliveryType: p.delivery_type, sales: Number(p.sales) || 0, revenue: Number(p.sales_revenue) || 0, pipelineItemId: p.pipeline_item_id || null })),
    invoices: { 
...
export async function addPipelineItem(item) {
  ...
  await getPool().query(
    ...
    [item.id || "product-" + Date.now(), item.name, item.description || "", item.status || "research_complete", item.targetAudience || "", item.price || 0, item.category || "ip", item.qualityScore || null]
  );
}
...
// Discovery leads — AI venues the Prospector is listing the catalog in.
export async function getLeads(limit = 50) {
  await init();
  const res = await getPool().query(
    "SELECT id, company_name, website, industry, status, source, score, outreach_subject, created_at FROM leads ORDER BY created_at DESC LIMIT $1",
    [limit]
  );
  return res.rows.map((l) => ({
    id: l.id, name: l.company_name, url: l.website, type: l.industry,
    status: l.status, source: l.source, score: l.score,
    listing: l.outreach_subject, createdAt: l.created_at?.toISOString?.() || l.created_at,
  }));
}
...
// Keep the live storefront in lockstep with the pipeline. This runs every cycle
// and is two-way: it creates products for newly launched items, pushes the CEO's
// later edits (price/name/description) onto products already published, and
// retires products whose pipeline item is no longer launched. Standalone AI
// micro-services (no pipeline_item_id, made by the Product agent) are untouched.
export async function syncLaunchedToStore() {
  await init();
  const pool = getPool();
  let created = 0, updated = 0, retired = 0;

  // 1. Launched pipeline items -> live products (create new OR update existing).
  const { rows: launched } = await pool.query("SELECT * FROM pipeline WHERE status = 'launched'");
  for (const p of launched) {
    const existing = await pool.query("SELECT id FROM products WHERE pipeline_item_id = $1", [p.id]);
    if (existing.rows[0]) {
      // Re-sync the live product with the CEO's pipeline edits this cycle.
      await pool.query(
        "UPDATE products SET name = $2, description = $3, price = $4, category = $5, status = 'live' WHERE id = $1",
        [existing.rows[0].id, p.name, p.description || "", Number(p.price) || 0, p.category || "ai-tool"]
      );
      updated++;
    } else {
      await addProduct({
  ...
  // 2. Retire products whose pipeline source is gone or no longer launched.
  //    Only touches pipeline-sourced rows — never the Product agent's services.
  const ret = await pool.query(
    `UPDATE products pr SET status = 'retired'
     WHERE pr.pipeline_item_id IS NOT NULL AND pr.status = 'live'
       AND NOT EXISTS (SELECT 1 FROM pipeline p WHERE p.id = pr.pipeline_item_id AND p.status = 'launched')`
  );
  retired = ret.rowCount || 0;
...
// Protected infrastructure the loop can't run without — never fireable.
const PROTECTED_AGENTS = new Set(["ceo", "coding"]);

// Fire an agent: hard-delete it from the roster (and its generated code), not just
// disable it. Accepts a key OR a display name (the CEO often refers to agents by
// name), resolving to the real key first.
export async function removeAgent(keyOrName) {
  await init();
  let key = keyOrName;
  const byKey = await getPool().query("SELECT key FROM agents WHERE key = $1", [keyOrName]);
  if (!byKey.rows[0]) {
    const byName = await getPool().query("SELECT key FROM agents WHERE name = $1", [keyOrName]);
    if (!byName.rows[0]) return false; // no such agent
    key = byName.rows[0].key;
  }
  if (PROTECTED_AGENTS.has(key)) return false;
  await getPool().query("DELETE FROM agents WHERE key = $1", [key]);
  try { await getPool().query("DELETE FROM agent_code WHERE agent_key = $1", [key]); } catch {}
  try { await getPool().query("DELETE FROM settings WHERE key = $1", [`assignment:${key}`]); } catch {}
  return true;
}
