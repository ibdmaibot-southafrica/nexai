...
export async function chat(systemPrompt, userMessage, options = {}) {
  ...
  if (CUSTOM_API_URL && CUSTOM_API_KEY) {
    return callCustomAPI(systemPrompt, userMessage, temperature, maxTokens);
  }

  // Priority 2: OpenRouter
  if (process.env.OPENROUTER_API_KEY) {
    return callOpenRouter(systemPrompt, userMessage, temperature, maxTokens);
  }

  // Priority 3: Local Ollama
  return callOllama(systemPrompt, userMessage, temperature, maxTokens);
}
...
async function callOpenRouter(systemPrompt, userMessage, temperature, maxTokens) {
  ...
  // Fallback routing: OpenRouter tries `models` in order and auto-falls-back when
  // a model errors or is rate-limited. Default fallback is openrouter/auto, so a
  // 429 on the free primary transparently routes to a working (paid) model.
  const fallback = process.env.OPENROUTER_FALLBACK_MODEL || "openrouter/auto";
  const routing = fallback && fallback !== model ? { models: [model, fallback] } : { model };
